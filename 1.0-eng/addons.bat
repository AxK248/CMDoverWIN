...
for /d %%d in ("%~dp0addons\*") do (
    ...
    if exist "%%~d\init.bat" (
        ...
        set "mod_name=Untitled"
        set "mod_author=Unknown"
        set "mod_ver=1.0"
        set "mod_code="
        set "mod_api="
...
if %mod_count% equ 0 echo [Addons not found or not compatible with the API %current_api_ver%]
echo.

set "user_choice="
set /p user_choice="Enter the code: "
...
:error
echo Access Denied. Invalid add-on access token.
...
:reload
echo Reload the page... && timeout /t 1 /nobreak >nul && goto addons
